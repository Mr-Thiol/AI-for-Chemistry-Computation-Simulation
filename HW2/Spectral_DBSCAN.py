...
def run_spectral_clustering_pipeline(csv_filepath: str):
    ...
    determine the optimal number of clusters, after filtering out DBSCAN outliers.
    """
    # ==========================================
    # 0. Data Loading & Filtering
    # ==========================================
    try:
        df_pca = pd.read_csv(csv_filepath)
    except FileNotFoundError:
        print(f"Error: Could not find {csv_filepath}. Run the DBSCAN script first.")
        return

    # Filter out DBSCAN outliers (where DBSCAN_Cluster == -1)
    if 'DBSCAN_Cluster' in df_pca.columns:
        initial_count = len(df_pca)
        df_pca = df_pca[df_pca['DBSCAN_Cluster'] != -1].copy()
        filtered_count = initial_count - len(df_pca)
        print(f"Filtered out {filtered_count} DBSCAN outliers.")
    else:
        print("Warning: 'DBSCAN_Cluster' column not found in the CSV. Proceeding without filtering.")
    ...
    print(f"Loaded {X_pca.shape[0]} normal molecules with {X_pca.shape[1]} Principal Components for clustering.")
    ...
    # Save to a new filename to avoid overwriting previous results
    output_csv = r"E:\JupyterPjs\AI4Chem\HW2\QM9_Spectral_Clustered_Filtered.csv"
    df_pca.to_csv(output_csv, index=False)
    print(f"Filtered Spectral Clustering results saved to {output_csv}")
    ...
    # The vertical line is plotted exactly at optimal_k to align with the peak
    ...
    axes[2].set_title(f'Spectral Clustering (Filtered, K={optimal_k})', fontsize=12)
    ...
    output_filename = r"E:\JupyterPjs\AI4Chem\HW2\Spectral_Clustering_Evaluation_Filtered.png"
    plt.savefig(output_filename, bbox_inches='tight')
    print(f"Figure saved successfully as {output_filename}")
    plt.show()

if __name__ == "__main__":
    # Updated input file to point to the DBSCAN clustered data
    csv_path = r"E:\JupyterPjs\AI4Chem\HW2\QM9_DBSCAN_Clustered.csv" 
