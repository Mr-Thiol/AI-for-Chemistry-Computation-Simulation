...
def train(config):
    #  Load config
    device      = config.global_setting.device           # cpu or cuda:0, cuda:1 etc.
    workdir     = config.global_setting.workdir          # e.g. ./logs
    config_path = config.global_setting.config_path      # absolute path to .yml
    config_name = config.global_setting.task_name        # e.g. qm9

    resume      = config.train.resume                    # bool
    resume_from = config.train.resume_path               # existing log dir when resume=True

    seed_all(config.global_setting.seed)

    #  Handle resume training 
    if resume:
        log_dir = get_new_log_dir(workdir, prefix=config_name, tag='resume')
        os.symlink(
            os.path.realpath(resume_from),
            os.path.join(log_dir, os.path.basename(resume_from.rstrip('/')))
        )
    else:
        log_dir = get_new_log_dir(workdir, prefix=config_name)
    ...
    #  Initialize logger and log writer
    ...
    if resume:
        # Always pick the latest checkpoint under resume_from/checkpoints/
        ckpt_path, start_iter = get_checkpoint_path(
            os.path.join(resume_from, 'checkpoints'), it=None
        )
        ...
        # Override LR if resume_lr is set in config
        if hasattr(config.train, 'resume_lr') and config.train.resume_lr is not None:
            for pg in optimizer_global.param_groups:
                pg['lr'] = config.train.resume_lr
            for pg in optimizer_local.param_groups:
                pg['lr'] = config.train.resume_lr
            logger.info('LR overridden to %.6f after resume' % config.train.resume_lr)
    ...
    def train(it):
        ...
        logger.info('[Train] Step %05d | Loss %.2f | Loss(Global) %.2f | Loss(Local) %.2f | Grad %.2f | LR(Global) %.6f | LR(Local) %.6f' % (
