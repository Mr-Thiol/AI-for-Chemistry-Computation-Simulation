# script_2_acsf_S1_to_S6_Full.py
import numpy as np
import pandas as pd
import pickle
import glob
import os
import scipy.special as sp  # 新增：用于计算球谐函数和勒让德多项式
...
def calculate_S2(nbr_info, df):
    """
    计算 S^2: (rc, nb, n, L) - 新增
    依赖球谐函数 Y_Lm，计算公式为 sum_m | sum_j R_n * Y_Lm |^2
    """
    features = []
    ...
    for _, row in df.iterrows():
        ...
        for m in range(-L, L + 1):
            ...
            for j in idx:
                ...
                r = dists[j]
                x, y, z = vecs[j]
                # 计算极角 phi [0, pi] 和 方位角 theta [-pi, pi]
                phi = np.arccos(np.clip(z / r, -1.0, 1.0))
                theta = np.arctan2(y, x)
                
                # 调用 scipy 的球谐函数: sph_harm(m, n, theta, phi)
...
def calculate_S5(nbr_info, df):
    """
    计算 S^5: (rc, nb1, nb2, L, n, m, p) - 新增
    包含勒让德多项式 P_L 的三体相互作用特征
    """
    features = []
    dists, syms, vecs = nbr_info['distances'], nbr_info['neighbor_symbols'], nbr_info['distance_vectors']
...
def calculate_S6(nbr_info, df):
    """计算 S^6: (rc, nb1, nb2, nb3, n, m, p, ζ, λ)"""
    features = []
    dists, syms, vecs = nbr_info['distances'], nbr_info['neighbor_symbols'], nbr_info['distance_vectors']
...
def load_csv_for_Sn(n):
    """自动寻找并读取匹配模式 Table_S*_S^n.csv 的文件"""
...
def main():
    ...
    # 动态加载所有 1-6 的 CSV 参数表
    df_s1 = load_csv_for_Sn('1')
    df_s2 = load_csv_for_Sn('2')  # 新增
    df_s3 = load_csv_for_Sn('3')
    df_s4 = load_csv_for_Sn('4')
    df_s5 = load_csv_for_Sn('5')  # 新增
    df_s6 = load_csv_for_Sn('6')

    ti_indices = [idx for idx, info in neighbor_data.items() if info['symbol'] == 'Ti']
    print(f"\n开始为 {len(ti_indices)} 个 Ti 原子全量计算 ACSF 特征 (S1~S6)...")
    ...
    for idx in ti_indices:
        ...
        # 按序拼接所有存在的特征向量
        if df_s1 is not None: current_feature.extend(calculate_S1(info, df_s1))
        if df_s2 is not None: current_feature.extend(calculate_S2(info, df_s2))
        if df_s3 is not None: current_feature.extend(calculate_S3(info, df_s3))
        if df_s4 is not None: current_feature.extend(calculate_S4(info, df_s4))
        if df_s5 is not None: current_feature.extend(calculate_S5(info, df_s5))
        if df_s6 is not None: current_feature.extend(calculate_S6(info, df_s6))
            
        all_features[idx] = np.array(current_feature)
        print(f"Ti 原子 {idx} 计算完成，当前全量特征总维度: {len(current_feature)}")
    
    # 修改保存的文件名，凸显全量特征
    save_filename = 'Ti_ACSF_Full_Features_S1toS6.npy'
    np.save(save_filename, all_features)
    print(f"\n✅ 所有特征计算完毕，已保存至 '{save_filename}'！")
