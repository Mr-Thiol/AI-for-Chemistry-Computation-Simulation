...
def calculate_S3(nbr_info, df):
    """计算 S^3: 依赖参数 (rc, nb1, nb2, n, m, ζ, λ)"""
    features = []
    ...
    for _, row in df.iterrows():
        ...
        for j in idx1:
            ...
            for k in idx2:
                ...
                term = (1.0 + lam * cos_theta)**zeta * R_n(dists[j], rc, n) * R_n(dists[k], rc, m)
                val += term
...
def calculate_S4(nbr_info, df):
    """计算 S^4: 依赖参数 (rc, nb1, nb2, n, m, p, ζ, λ)"""
    features = []
    dists, syms, vecs = nbr_info['distances'], nbr_info['neighbor_symbols'], nbr_info['distance_vectors']
    ...
    for _, row in df.iterrows():
        rc = float(row['rc'])
        nb1, nb2 = str(row['nb1']).strip(), str(row['nb2']).strip()
        n, m, p = float(row['n']), float(row['m']), float(row['p'])
        zeta = float(get_col(row, 'ζ', 'zeta'))
        lam = float(get_col(row, 'λ', 'lam', 'lambda'))
        ...
        for j in idx1:
            ...
            for k in idx2:
                ...
                cos_theta = calc_cos_theta(vecs[j], vecs[k], dists[j], dists[k])
                term = (1.0 + lam * cos_theta)**zeta * R_n(dists[j], rc, n) * R_n(dists[k], rc, m) * R_n(r_jk, rc, p)
                val += term
...
def calculate_S6(nbr_info, df):
    """计算 S^6: 依赖参数 (rc, nb1, nb2, nb3, n, m, p, ζ, λ)"""
    features = []
    dists, syms, vecs = nbr_info['distances'], nbr_info['neighbor_symbols'], nbr_info['distance_vectors']
        ...
        for j in idx1:
            ...
            for k in idx2:
                ...
                for l in idx3:
                    ...
                    term = (1.0 + lam * cos_delta)**zeta * R_n(dists[j], rc, n) * R_n(dists[k], rc, m) * R_n(dists[l], rc, p)
                    val += term
...
def main():
    ...
    print(f"\n开始为 {len(ti_indices)} 个 Ti 原子计算 ACSF 特征 (S1, S3, S4, S6)...")
    ...
    for idx in ti_indices:
        ...
        # 根据是否成功加载 DataFrame，决定是否调用对应的计算函数
        ...
        print(f"Ti 原子 {idx} 计算完成，当前特征总维度: {len(current_feature)}")
    
    np.save('Ti_ACSF_features_Modular.npy', all_features)
    print("\n✅ 所有特征计算完毕，已保存至 'Ti_ACSF_features_Modular.npy'！")
